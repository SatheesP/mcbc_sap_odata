CLASS zcl_mcbc_meal_airline4_dpc_ext DEFINITION
  PUBLIC
  INHERITING FROM zcl_mcbc_meal_airline4_dpc
  CREATE PUBLIC .

  PUBLIC SECTION.
  PROTECTED SECTION.

    METHODS airlines_get_entityset REDEFINITION.
    METHODS airlines_get_entity REDEFINITION.
...
CLASS ZCL_MCBC_MEAL_AIRLINE4_DPC_EXT IMPLEMENTATION.


  METHOD airlines_get_entity.

    DATA ls_airline TYPE zcl_mcbc_meal_airline4_mpc=>ts_airline.
...
*-- Reads a unique row based on keys passed in URI
  ...
  METHOD airlines_get_entityset.
    ...
    DATA lt_airlines TYPE zcl_mcbc_meal_airline4_mpc=>tt_airline.
...
*-- /EntitySetName?$format=json&$inlinecount=allpages
    IF io_tech_request_context->has_inlinecount( ).
      es_response_context-inlinecount = lines( lt_airlines ).
    ENDIF.
...
*-- $top & $skip
  ...
  METHOD mealset_get_entity.
    ...
    DATA ls_meal TYPE zcl_mcbc_meal_airline4_mpc=>ts_meal.
...
*-- Reads a unique row based on keys passed in URI
  ...
  METHOD mealset_get_entityset.
    ...
    DATA lt_meals TYPE zcl_mcbc_meal_airline4_mpc=>tt_meal.
...
*-- Implement Associations
    DATA(lt_navpath) = io_tech_request_context->get_navigation_path( ).
    IF line_exists( lt_navpath[ source_entity_type = zcl_mcbc_meal_airline4_mpc=>gc_airline ] ) AND
        line_exists( lt_navpath[ nav_prop = 'TOMEALS'  ] ).
      DATA(lt_skeys) = io_tech_request_context->get_source_keys( ).
      IF line_exists( lt_skeys[ name = 'AIRLINEID' ] ).
        IF where_cond IS INITIAL.
          where_cond = |AIRLINEID = '{ lt_skeys[ name = 'AIRLINEID' ]-value }'|.
        ELSE.
          where_cond = |{ where_cond } AND AIRLINEID = '{ lt_skeys[ name = 'AIRLINEID' ]-value }'|.
        ENDIF.
      ENDIF.
    ENDIF.
